/**
 * Stock Audit Log Utility Functions
 * Standalone utility functions for creating stock audit logs
 * These functions match the API of the deleted legacy service
 */

import { StockAuditLogRepository } from "../../repositories/stockAuditLog.repository.js";
import { STOCK_AUDIT_ACTION } from "../../types/stockAuditLog.types.js";
...
export const createStockAuditLog = async ({
  ...
  // Use repository to create the audit log
  const repository = new StockAuditLogRepository();
  const auditLog = await repository.create(auditLogData, {
    session,
  });
